...
const dir = 'src/app/api/agents/tasks/[id]/complete';
fs.mkdirSync(dir, { recursive: true });
fs.writeFileSync(dir + '/route.ts', completeRoute, 'utf8');
console.log('Written complete/route.ts:', fs.statSync(dir + '/route.ts').size, 'bytes');

// Delete old conflicting folders
function rmrf(p) { if (fs.existsSync(p)) fs.rmSync(p, { recursive: true, force: true }); }
rmrf('src/app/api/agents/tasks/[agentId]');
rmrf('src/app/api/agents/tasks/[taskId]');
console.log('Deleted [agentId] and [taskId]');

const remaining = fs.readdirSync('src/app/api/agents/tasks');
console.log('Remaining dirs:', remaining);
