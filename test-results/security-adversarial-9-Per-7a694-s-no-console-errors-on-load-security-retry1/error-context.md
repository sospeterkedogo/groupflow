...
- Name: security-adversarial.spec.ts >> 9. Performance & Availability (SLA Targets) >> Landing page has no console errors on load
- Location: tests\security-adversarial.spec.ts:545:7

# Error details

```
Error: Console errors: Failed to load resource: the server responded with a status of 500 (Internal Server Error)

expect(received).toHaveLength(expected)

Expected length: 0
Received length: 1
Received array:  ["Failed to load resource: the server responded with a status of 500 (Internal Server Error)"]
```
