# BRAND IDENTITY — OFFICIAL BUSINESS DOCUMENTATION
# FlowSpace (Working Title) → Final Brand Selection Required

---

## PART I: CANDIDATE BRAND NAMES

### Option 1 — **SYNAPSE**
**Domain:** synapse.edu / trysynapse.com / usesynapse.io  
**Tagline:** *"Where minds connect and work gets done."*  
**Identity:** Synapse evokes the neural connection — fast, precise, and essential. It positions
the platform as the connective tissue between students, educators, and institutions. Scientific
authority, clean aesthetic.  
**Logo Concept:** Two nodes connected by a glowing arc. Dark background. Single brand colour —
electric blue (#3B82F6) or emerald green (existing brand). Wordmark in tight geometric sans-serif.  
**Personality:** Intelligent. Precise. Institutional. Trusted.  
**Best for:** University-facing enterprise positioning; grant applications; STEM audiences.

---

### Option 2 — **NEXORA**
**Domain:** nexora.io / nexora.app  
**Tagline:** *"The next generation of academic collaboration."*  
**Identity:** NEXORA blends "next," "nexus," and "ora" (Latin: speak/time). Modern, invented,
memorable, and globally pronounceable in every major language market.  
**Logo Concept:** Lowercase wordmark with a subtle gradient sweep on the final 'a'. Optional mark:
a stylised double-arrow forming a meeting point. Works on dark and light backgrounds.  
**Personality:** Ambitious. Global. Student-first. Forward-looking.  
**Best for:** Consumer-facing launch; social media virality; Gen Z audience.

---

### Option 3 — **FLOWDESK**
**Domain:** flowdesk.app / flowdesk.io  
**Tagline:** *"Your team. Your work. Your credit."*  
**Identity:** Evokes a workspace — a "desk" where collaboration flows naturally. Approachable,
productivity-oriented, and pairs well with the existing FlowSpace brand DNA.  
**Logo Concept:** Rounded wordmark. Flowing underline that transitions from a muted start colour
to brand-green. Simple, instantly legible at small sizes (favicon, app icon).  
**Personality:** Approachable. Organised. Productive. Empowering.  
**Best for:** Secondary/further education; teacher-facing rollout; Nile LMS integration context.

---

### Option 4 — **COLLAB.SPACE** / **COLLABSPACE**
**Domain:** flowspace.app / flowspace.io  
**Tagline:** *"One space. Every contribution. Perfect clarity."*  
**Identity:** Direct, self-describing, strong for SEO. Immediately communicates function.
Less abstract than options 1–3; excellent for institutional procurement conversations.  
**Logo Concept:** Square tile with a subtle inner glow — the "space" made visible. Monochrome
with accent. Pairs well with formal university branding guidelines.  
**Personality:** Clear. Professional. Functional. Trustworthy.  
**Best for:** Enterprise/institutional sales; compliance-heavy markets; Africa/MENA expansion.

---

### Option 5 — **ARCTIFY**
**Domain:** arctify.com / arctify.io  
**Tagline:** *"Arc your potential. Amplify your team."*  
**Identity:** ARCTIFY is energetic, invented, and globally trademarkable with zero conflicts.
"Arc" suggests trajectory, growth, and the graph of contribution over time — perfect metaphor
for the platform's analytics core.  
**Logo Concept:** Bold geometric uppercase wordmark. The 'A' replaced by a rising arc glyph.
Electric-green (#10B981) on black. Versatile across merch, apps, and display advertising.  
**Personality:** Bold. Energetic. High-performance. Premium.  
**Best for:** Marketing campaigns; social virality; brand merchandise; investor decks.

---

## PART II: RECOMMENDED SELECTION

**NEXORA** or **ARCTIFY** are the strongest candidates for a global consumer launch targeting
students. NEXORA has the broader appeal; ARCTIFY has the highest visual brand potential.

**SYNAPSE** is recommended if the primary go-to-market is institutional (university procurement,
government education grants, LMS partnerships).

---

## PART III: BRAND IDENTITY SYSTEM (apply to chosen name)

### Colour Palette
| Token         | Value     | Usage                                   |
|---------------|-----------|----------------------------------------|
| Brand Primary | `#10B981` | CTAs, links, active states, highlights  |
| Brand Dark    | `#059669` | Hover states, gradients                 |
| Background    | `#0A0A0A` | Page backgrounds                        |
| Surface 1     | `#111111` | Cards, panels                           |
| Surface 2     | `#1A1A1A` | Elevated elements                       |
| Border        | `rgba(255,255,255,0.08)` | Subtle dividers                |
| Text Primary  | `#F3F4F6` | Headings                                |
| Text Secondary| `rgba(255,255,255,0.55)` | Body text                    |
| Text Muted    | `rgba(255,255,255,0.3)`  | Labels, captions               |

### Typography
- **Display / Hero:** Custom weight 950, letter-spacing -0.05em, tight leading (0.95)
- **Heading:** Weight 800, letter-spacing -0.03em
- **Body:** Weight 500, letter-spacing -0.01em, line-height 1.65
- **Label/Tag:** Weight 900, UPPERCASE, letter-spacing 0.18em, size 0.68–0.75rem
- **Font family:** System — Inter, SF Pro Display, Segoe UI Variable, sans-serif

### Logo Construction Rules
1. **Clearspace:** Minimum clearspace = 1× the cap-height of the wordmark on all sides
2. **Minimum size:** 120px wide on digital; 20mm on print
3. **Do not:** stretch, recolour to anything outside palette, add shadows, use on busy backgrounds
4. **Icon mark:** The standalone icon (nodes/arc/glyph) may be used at 32×32px and smaller
5. **Dark mode first:** The primary lockup is always brand-on-black. A white-on-brand variant
   exists for light backgrounds only.

### Motion Principles
- Entry animations: `opacity 0 → 1`, `y: 20px → 0`, duration 0.7–0.9s, ease-out
- Micro-interactions: 150ms transitions, opacity + scale
- Loading states: skeleton shimmer in #111 → #1A1A1A

---

## PART IV: PLATFORM POSITIONING STATEMENT

> **"[BRAND NAME] is the institutional-grade collaboration platform built for the real dynamics
> of academic group work. We give every student a transparent, verifiable record of their
> individual contribution — and give educators the real-time visibility to assess fairly.
> We are not a chat tool. We are not a project management app. We are the academic operating
> system for the team."**

### Competitor Differentiation
| Platform        | What they do                        | Why we are different              |
|-----------------|-------------------------------------|-----------------------------------|
| Nile LMS        | Course delivery, content management | We sit *inside* the workflow, not above it |
| Canvas          | Grade management, submission        | We track HOW the work happened, not just the outcome |
| Notion/Trello   | Generic project management          | We are student-identity-aware, grade-linked, and institution-integrated |
| Google Workspace| File collaboration                 | We have contribution intelligence, not just document access |
| GitHub          | Code collaboration                  | We extend to all work types (design, writing, research) |

### School / Nile Integration Statement
> "FlowSpace integrates natively with Nile and other institutional LMS platforms via LTI 1.3.
> Students log in with their existing institutional credentials. Educators see FlowSpace
> contribution data directly alongside their Nile gradebook. No duplicate accounts.
> No IT configuration. 15-minute setup per institution."

---

## PART V: FOUNDING MEMBER PROGRAMME

Every pre-registered user receives:
- **Founding Member badge** — permanent, displayed on their profile
- **Priority access** — first 48 hours of open beta before public launch
- **Free plan preserved** — guaranteed free tier, locked in even if pricing changes post-launch
- **Name in credits** — optional listing in the platform's founding contributors page
- **Founder's Certificate** — a verifiable digital credential (future: blockchain-anchored)

---

## PART VI: VERSION ROADMAP

| Version  | Name              | Target Date    | Key Deliverables                               |
|----------|-------------------|----------------|------------------------------------------------|
| v1.0     | Foundation        | Current        | Core task management, contribution tracking, Stripe, auth |
| v1.5     | Institutional     | Q3 2025        | Admin panel v2, LMS bridge (Nile/Canvas pilot), analytics |
| v2.0     | Intelligence      | Q1 2026        | AI Study Coach, integrity engine, mobile apps  |
| v2.5     | Network           | Q3 2026        | Global knowledge network, peer discovery, credentials |
| v3.0     | Enterprise        | 2027+          | Multi-tenant institutional licensing, white-labelling, API |
